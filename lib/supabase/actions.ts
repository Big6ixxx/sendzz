...
export async function recordTransfer(params: {
  ...
  try {
    ...
    if (!sender) {
      console.warn(
        `[Supabase] Sender ${params.senderEmail} not found in users table. Skipping record.`,
      );
      return;
    }

    console.log(
      `[Supabase] Resolved IDs - Sender: ${sender.id}, Recipient: ${recipient?.id || 'NULL'}`,
    );
    ...
    if (insertError) {
      console.error(
        '[Supabase Actions] Failed to record transfer:',
        insertError,
      );
    } else {
    ...
    console.error(
      '[Supabase Actions] Critical failure in recordTransfer:',
      err,
    );
  }
}
...
export async function recordDeposit(params: {
  ...
  try {
    ...
    if (userError || !user) {
      console.error(
        `[Supabase] recordDeposit: User not found for email ${params.userEmail}`,
        userError,
      );
      return;
    }
    ...
    console.log(
      `[Supabase] recordDeposit: Found user ID ${user.id}. Attempting insert...`,
    );
...
export async function updateDepositStatus(
  ...
  try {
    ...
    console.log(
      `[Supabase] Deposit ${paycrestTxId} status updated to ${status}`,
    );
  } catch (err) {
  ...
  try {
    ...
    if (insertError) {
      if (insertError.code === '23503') {
        console.error(
          `[Supabase] WITHDRAWAL FAILED: Foreign key violation. The 'withdrawals' table is likely pointing to 'auth.users' instead of 'public.users'.`,
        );
      } else {
        console.error(
          '[Supabase Actions] Failed to record withdrawal:',
          insertError.message,
        );
      }
      return;
    ...
    console.log(
      `[Supabase] Withdrawal recorded successfully: ${params.paycrestOrderId}`,
    );
  } catch (err) {
    console.error(
      '[Supabase Actions] Critical failure in recordWithdrawal:',
      err,
    );
  }
}
...
export async function recordBridgeTransaction(params: {
  ...
  try {
    ...
    if (userError || !user) {
      console.error(
        `[Supabase Actions] User not found for bridge record: ${params.userEmail}`,
      );
      return;
    }

    console.log(`[Supabase Actions] Recording bridge for user ${user.id}`);
    ...
    console.error('[Supabase Actions] Failed to record bridge tx:', err);
  }
}
...
export async function updateBridgeStatus(
  ...
  try {
    ...
    console.error('[Supabase Actions] Failed to update bridge status:', err);
  }
}
...
export async function getUserActivities(userEmail: string) {
  try {
    ...
    const { data: bridges } = await supabaseAdmin
      .from('bridge_transactions')
      .select('*')
      .eq('user_id', internalId);

    const mapTransfer = (
      t: Database['public']['Tables']['transfers']['Row'] & {
        sender?: unknown;
      },
    ) => {
      const senderObj = t.sender as { email: string } | null;
      return {
        ...t,
        sender_email: t.sender_email || senderObj?.email || 'Unknown Sender',
...
export async function getAdminStats(adminEmail: string) {
  ...
  const confirmedWithdrawals =
    w?.filter((x) => x.status === 'completed' || x.status === 'processing') ||
    [];
  const confirmedTransfers = t?.filter((x) => x.status === 'completed') || [];

  const totalDeposits = confirmedDeposits.reduce(
    (acc, curr) => acc + (Number(curr.amount_usdc) || 0),
    0,
  );
  const totalWithdrawals = confirmedWithdrawals.reduce(
    (acc, curr) => acc + (Number(curr.amount_usdc) || 0),
    0,
  );
  const totalTransfers = confirmedTransfers.reduce(
  ...
  const pendingDepositsCount =
    d?.filter((x) => x.status === 'pending').length || 0;
  const pendingWithdrawalsCount =
    w?.filter(
      (x) => x.status === 'processing' || x.status === 'awaiting_verification',
    ).length || 0;
...
export async function getAdminTransactions(
  ...
  const all: AdminTransaction[] = [
    ...
    ...(deposits || []).map((d) => ({
      ...d,
      tx_type: 'deposit' as const,
      amount: d.amount_usdc || 0,
    })),
    ...(withdrawals || []).map((w) => ({
      ...w,
      tx_type: 'withdrawal' as const,
      amount: w.amount_usdc || 0,
    })),
  ].sort(
    (a, b) =>
...
export async function getAdminUsers(adminEmail: string) {
  ...
  const [{ data: d }, { data: w }, { data: t }] = await Promise.all([
    ...
    supabaseAdmin
      .from('transfers')
      .select(
        'sender_id, sender_email, recipient_id, recipient_email, amount, status',
      ),
  ]);
  ...
  const usersWithVolume = users.map((user) => {
    ...
    // Deposits
    const userDeposits =
      d?.filter((x) => x.user_id === user.id && x.status === 'confirmed') || [];
    const total_deposits = userDeposits.reduce(
      (a, b) => a + (Number(b.amount_usdc) || 0),
      0,
    );
    ...
    const userWithdrawals =
      w?.filter(
        (x) =>
          x.user_id === user.id &&
          (x.status === 'completed' || x.status === 'processing'),
      ) || [];
    const total_withdrawals = userWithdrawals.reduce(
      (a, b) => a + (Number(b.amount_usdc) || 0),
      0,
    );

    // Transfers
    const userTransfersSent =
      t?.filter(
        (x) =>
          (x.sender_id === user.id ||
            x.sender_email?.toLowerCase() === userEmail) &&
          x.status === 'completed',
      ) || [];
    const total_sent = userTransfersSent.reduce(
      (a, b) => a + (Number(b.amount) || 0),
      0,
    );

    const userTransfersReceived =
      t?.filter(
        (x) =>
          (x.recipient_id === user.id ||
            x.recipient_email?.toLowerCase() === userEmail) &&
          x.status === 'completed',
      ) || [];
    const total_received = userTransfersReceived.reduce(
      (a, b) => a + (Number(b.amount) || 0),
      0,
    );
    ...
    return {
      ...user,
      total_volume:
        total_deposits + total_withdrawals + total_sent + total_received,
      total_deposits,
      total_withdrawals,
      total_sent,
      total_received,
...
export async function getAdminAnalytics(
  ...
  const chartData: Record<
    string,
    { date: string; volume: number; users: number }
  > = {};
  ...
  const validTransfers = (transfers || [])
    .filter((x) => x.status === 'completed' && x.created_at)
    .map((x) => ({ a: Number(x.amount), d: x.created_at }));
  const validDeposits = (deposits || [])
    .filter((x) => x.status === 'confirmed' && x.created_at)
    .map((x) => ({ a: Number(x.amount_usdc), d: x.created_at }));
  const validWithdrawals = (withdrawals || [])
    .filter(
      (x) =>
        (x.status === 'completed' || x.status === 'processing') && x.created_at,
    )
    .map((x) => ({ a: Number(x.amount_usdc), d: x.created_at }));
