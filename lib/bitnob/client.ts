...
export class BitnobClient {
  ...
  async createCheckout(req: BitnobCheckoutRequest) {
    ...
    if (!res.ok) {
        ...
        if (res.status === 403) {
            throw new Error(`Bitnob 403 Forbidden: Your IP may not be whitelisted or your API key lacks permissions. Check your Bitnob Dashboard Settings. Details: ${errorText}`);
        }
