 ...
 *   FEE_TREASURY_<CHAIN>   — where the fee is collected, per settlement chain (shared with
 *                            withdrawals; resolved by lib/ramp/fees.ts so there is one map)
 *
...
import { treasuryFor } from '@/lib/ramp/fees';

/**
 * Fee-collection addresses per chain — the same treasury the withdrawal fee goes to, resolved
 * through the SAME lookup so the two cannot drift.
 *
 * This used to be a second hardcoded copy of the address map, which meant a chain added to one
 * and not the other would collect bridge fees while silently skipping withdrawal fees, or the
 * reverse. One function, one answer.
 *
 * Server-only (no NEXT_PUBLIC prefix), so the browser can't read the addresses. The fee is
 * collected in a client-built user operation, so the address is handed over by the
 * `quotePlatformFee` server action — exactly how executeOffRamp embeds the treasury in the
 * order it returns.
 */
...
export function getFeeTreasury(chain: string): string | null {
  return treasuryFor((chain || '').toLowerCase()) ?? null;
}
