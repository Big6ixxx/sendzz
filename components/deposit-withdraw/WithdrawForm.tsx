  ...
  if (hook.step === 1) {
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        >
          {hook.loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Continue"}
        </button>
      </div>
    );
  }
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="p-4 bg-muted/30 rounded-2xl border border-border space-y-3">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold">1 USDC = {hook.quote.rate.toLocaleString()} NGN</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Estimated Received</span>
            <span className="font-bold text-foreground">{hook.quote.payoutAmount.toLocaleString()} NGN</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <BankSelector
            ...
            onSelect={(b) => hook.setBankDetails({ ...hook.bankDetails, bankCode: b.code, bankName: b.name, accountName: '' })}
            accountNumber={hook.bankDetails.accountNumber}
            onAccountNumberChange={(val) => hook.setBankDetails({ ...hook.bankDetails, accountNumber: val, accountName: '' })}
        ...
        <button
          ...
          className="btn-primary w-full gap-2"
        >
          {hook.loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Confirm Withdrawal"}
        </button>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-center">
        ...
        <div className="space-y-2">
          <h3 className="text-2xl font-black uppercase tracking-tighter">Ready to Send</h3>
          <p className="text-sm text-muted-foreground">Please confirm the transfer of funds to complete your withdrawal.</p>
        </div>
