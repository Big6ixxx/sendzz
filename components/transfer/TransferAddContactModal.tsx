...
interface TransferAddContactModalProps {
  isOpen: boolean;
  onClose: () => void;
  senderEmail: string;
  defaultEmail: string;
...
export function TransferAddContactModal({
  isOpen,
  onClose,
  senderEmail,
  defaultEmail,
}: TransferAddContactModalProps) {
  const [newContactName, setNewContactName] = useState('');
  const [newContactEmail, setNewContactEmail] = useState(defaultEmail);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-6">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative w-full max-w-md card-glass p-8 bg-[#0a0a0b] animate-in fade-in zoom-in-95 duration-300">
        <button
          onClick={onClose}
