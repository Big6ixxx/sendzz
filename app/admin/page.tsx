...
import { getAdminStats, getAdminAnalytics } from '@/lib/supabase/actions';
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-display font-bold text-white tracking-tight mb-2">Platform Overview</h1>
          <p className="text-white/40 font-medium">Monitoring real-time performance and user activity</p>
        </div>
            ...
            {(['7d', '30d', 'all'] as const).map((p) => (
              <button
                ...
                className={cn(
                  ...
                  period === p ? "bg-white/10 text-white shadow-xl" : "text-white/30 hover:text-white"
                )}
              >
                {p}
              </button>
        ...
        {statCards.map((stat, i) => (
          ...
          >
            ...
            <div className="space-y-1">
              <p className="text-xs font-bold text-white/30 uppercase tracking-[0.2em]">{stat.title}</p>
              <h3 className="text-3xl font-display font-bold text-white tracking-tighter">
                {statsLoading ? <div className="h-9 w-24 bg-white/5 animate-pulse rounded-lg" /> : stat.value}
              </h3>
              <p className="text-[10px] font-medium text-white/20 uppercase tracking-wider pt-1">{stat.desc}</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-white/40 uppercase tracking-[0.2em]">Transaction Volume</h3>
            </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-white/40 uppercase tracking-[0.2em]">User Onboarding</h3>
          </div>
          <div className="card-glass p-8 h-[400px]">
            {analyticsLoading ? (
               <div className="w-full h-full flex items-center justify-center">
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xs font-bold text-white/40 uppercase tracking-[0.2em]">Activity Breakdown</h3>
        </div>
            ...
            {subMetrics.map((item) => (
              <div key={item.label} className="space-y-4">
                ...
                <p className="text-3xl font-display font-bold text-white tracking-tight">
                  {statsLoading ? <div className="h-8 w-20 bg-white/5 animate-pulse rounded-lg" /> : item.value}
                </p>
                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                  <motion.div 
                    ...
                    className={cn("h-full shadow-[0_0_10px_rgba(0,0,0,0.5)]", item.barColor)}
                  />
                </div>
              </div>
            ))}
