    ...
    return ((data || []) as AdminTransaction[]).filter((tx) => {
      ...
      return (
        ...
        (tx.tx_type === 'withdrawal' &&
          tx.bank_account_masked.toLowerCase().includes(searchLower)) ||
        ('user_id' in tx && tx.user_id?.toLowerCase().includes(searchLower))
      );
    });
  }, [data, search]);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-white/40 mt-1 font-medium">
            Monitoring platform flow. Showing 20 most recent per type.
          </p>
        </div>
      ...
      <div className="card-glass p-0 overflow-hidden border-white/5">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-white/2">
                <th className="px-6 py-4 text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">
                  Type
                </th>
                <th className="px-6 py-4 text-[10px] font-bold text-white/30 uppercase tracking-[0.2em]">
                  Entity
            ...
            <tbody className="divide-y divide-white/5">
              {isLoading ? (
                ...
                (currentItems as AdminTransaction[]).map((tx, i: number) => (
                  <motion.tr
                    key={tx.id}
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-[10px] font-bold text-white/20 tracking-tighter uppercase">
                          USDC
                        </span>
                      </div>
                    </td>
        ...
        <div className="px-6 py-4 bg-white/2 border-t border-white/5 flex items-center justify-between">
          <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">
            Showing {currentItems.length} of {filteredTransactions.length}{' '}
            results
          </p>
